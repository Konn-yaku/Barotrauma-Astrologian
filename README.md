...
| 医疗技能 | **+45** |
| 自身伤害 | 随医疗技能阶梯提升，**+4% ~ +20%** |
| 队友伤害 | 1000 范围内 **+5%**（离开范围约 4 秒后失效） |

### 伤害阶梯怎么来的

写了 5 个 `OnAttack` 组：基础一个无条件 +4%，
另 4 个各带一道递增的医疗门槛，每个也是 +4%。
...
| 来源 | 伤害加成 | 范围 |
|---|---|---|
| 本模组「太阳神之衡」 | +15~20% | 全队·永久 |
| 原版安全官 `implacable` | +25% | 仅自己·限时 |
| 原版舰长 `steadfast` | +30% | 仅自己·限时 |
| **占卜（自己）** | **+4~20%** | 仅自己·常驻 |
| **占卜（队友）** | **+5%** | 1000 内 |
...
- 闪电巫师（`lightningwizard`）的实际效果是
  「10 秒内未受攻击，则下一次近战攻击减免 80%，并眩晕攻击者 4 秒」，
  **不提供任何技能点**。
- 原版所有技能加点的最大值是 **+30**（医生的 `medicalexpertise`，
  已经被我们用在了「天星交错」上）。

目前按你的要求实装为 **+45**。如果想收回原版上限内，
把 `Talents/AstrologianTalents.xml` 里的 `value="45"` 改成 `30` 即可（一处）。
