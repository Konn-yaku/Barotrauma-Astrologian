# Astrologian 占星术士

《潜渊症》(Barotrauma) 的职业模组。加入一个以《最终幻想14》**占星术士**（Astrologian）为主题的治疗职业，
职业定位取自原版**医生**（Medical Doctor）。

> **开发中 (WIP)**

## 特点

- **治疗者定位** — 主技能为医疗，偏向艇内支援
- **纯 XML 实现** — 不需要 Lua / LuaCsForBarotrauma
- **中英双语**

## 完成度

| 内容 | 状态 |
|---|---|
| 职业本体（技能 / 初始装备 / AI 行为 / 指令） | ✅ |
| 占星魔法（5 个天赋） | ✅ |
| 奥秘卡（抽卡制：2 个抽卡道具 + 6 张卡） | 🚧 物品已就绪，天赋待重做 |
| 会奶人的DPS（4 个天赋 + 4 件专属装备） | ✅ |
| 初级天赋 | ✅ |
| 职业图标 | ✅ 本地素材 |
| 物品贴图 | ✅ 本地素材 |

## 初级天赋

开局即可点选，共 10 个。

| 天赋 | 效果 |
|---|---|
| **星图** | 医疗物品价格 −10%；每回合生命值首次低于 30% 时获得 100 马克 |
| **星位合图** | 你治疗队友时，自己也缓缓恢复 |
| **擢升** | 解锁「自动注射器耳机」配方 |
| **重力** | 解锁「压力稳定器」配方 |
| **凶星** | 你造成的伤害 +10% |
| **命运之轮** | 最大生命 +20% |
| **天星交错** | 对鸦片类与化学成瘾的抗性各 +50% |
| **生辰** | 心肺复苏效力翻倍 |
| **求生意志** | 为队友提供的心肺复苏效果 +20% |
| **消防搬运** | 拖拽角色时移动不受限 |

> 「命运之轮」「天星交错」「生辰」目前挂的是通用医疗兵效果，之后会换成与名字相符的效果。

## 占星魔法

| 层 | 天赋 | 效果 |
|---|---|---|
| 1 | **（占位符）** | 效果待重新设计 |
| 2 | **先天禀赋** | 目标生命值低于 30% 时，你的治疗效果提升至 2.2 倍 |
| 2 | **光速** | 潜艇上有人失去知觉时，你获得 40% 移动速度加成；拖拽其他角色时不再减速 |
| 3 | **阳星合相** | 你在潜艇内时，1000 范围内的船员持续缓慢自愈（不断减轻伤害与出血） |
| 4 | **星天开门** | 进入任务时全船各获得一张庇护；各自倒地时自动触发，恢复满血后消耗 |

## 奥秘卡

抽卡制：**每 60 秒抽一次，星极 / 灵极交替**。一次抽 3 张，正好是一个极的整个卡池。

| 极 | 抽卡道具 | 卡池 |
|---|---|---|
| 星极 | **星极抽卡** | 太阳神之衡 · 放浪神之箭 · 建筑神之塔 |
| 灵极 | **灵极抽卡** | 战争神之枪 · 世界树之干 · 河流神之瓶 |

**流程**

1. 天赋交替发放「星极抽卡」/「灵极抽卡」
2. **按住右键使用它** → 该极的 3 张卡进入它自己的容器（随身只占 1 格）
3. 把要用的卡拖出来，**按住右键打出**
4. 抽卡道具在第一次使用后 60 秒自毁，容器里没用掉的卡一并销毁
5. 天赋检测到道具消失 → 发放另一个极的抽卡道具

| 卡 | 效果 |
|---|---|
| **太阳神之衡** | 全队伤害 +25% |
| **战争神之枪** | 全队近战攻速 +30% / 远程攻速 +20% |
| **放浪神之箭** | 全队移速 +25% / 游泳速度 +35% |
| **世界树之干** | 全队伤害抗性 +25% |
| **建筑神之塔** | 全队维修速度与焊接船体速度 +35% |
| **河流神之瓶** | 全队氧气消耗 −25% |

> ⚠️ **过渡状态**：
> 1. 抽卡道具与 6 张卡已经做好，但天赋**还在发放旧的「被动全队常驻」版本**；
> 2. 卡打出去目前只作用于使用者自己，还没接天赋广播（目标是无视距离给全队）。
> 数值与天赋树会随重做一起调整，现列数值仅供参考。

## 会奶人的DPS

| 档 | 天赋 | 效果 |
|---|---|---|
| 1 | **天球仪** | 获得手持远程武器「天球仪」 |
| 2 | **地星** | 获得「地星」，并解锁它的制造配方 |
| 3 | **（占位符）** | 效果待重新设计 |
| 4 | **占卜** | 医疗技能 +45；自身伤害 +10%~+30%；附近队友 +10% 伤害 |

### 天球仪

手持远程武器，发射即时光束，不需要弹药。可收进随身物品栏，要用时取出。

| 属性 | 值 |
|---|---|
| 类别 | 小型物品 |
| 持握 | 双手 |
| 射速 | `reload 0.8` |
| 弹药 | 不需要 |

| 项 | 天球仪 | 快速裂变加速器 |
|---|---|---|
| 结构伤害 | 17 | 35 |
| 物品伤害 | 17 | 35 |
| 主要伤害 | 25 | 50 |
| 断肢概率 | 25% | 50% |
| 附加效果 | 焚灼（烧伤） | 辐射病 25 |

### 地星

放在地上，需 10 秒蓄力。蓄力完成后，只要它所在的舱室开始进水，
就会释放一次范围 250 的船体修复，随后消失。

一颗只能生效一次，要再用需在制造台重新制作（30 秒 + 1 精炼钍 + 2 塑胶）。

### 命运之轮

投掷物。落地即碎，为范围内所有角色提供 20 秒的 65% 全伤害抗性，
并持续减轻伤势与出血。

范围 400，不区分敌我。一次性消耗品。

### 占卜

| 项 | 值 |
|---|---|
| 医疗技能 | +45 |
| 自身伤害 | 随医疗技能提升（见下表） |
| 队友伤害 | 1000 范围内 +10%，离开范围约 4 秒后失效 |

| 医疗技能 | < 40 | ≥ 40 | ≥ 60 | ≥ 80 | ≥ 100 |
|---|---|---|---|---|---|
| 伤害 | +10% | +15% | +20% | +25% | +30% |

## 安装

1. 将 `Astrologian` 文件夹放入游戏的 `LocalMods` 目录
   （通常为 `<Steam>/steamapps/common/Barotrauma/LocalMods/`）
2. 启动游戏 → **设置 → 模组** → 启用 `Astrologian`
3. 新建战役时即可在船员界面选择「占星术士」

> ⚠️ 本仓库**不含图片素材**，`Textures/` 已被 `.gitignore` 排除。
> 从仓库 clone 后需自备 `Textures/` 目录（结构与文件名见下），
> 否则职业图标、物品贴图与天赋/增益图标会显示为空。

> 修改任何文件后**必须完全重启游戏**才会生效。

## 文件结构

```
Astrologian/
├─ filelist.xml                          内容包清单（模组入口）
├─ Jobs/AstrologianJob.xml               职业定义
├─ TalentTrees/AstrologianTalentTrees.xml 天赋树结构
├─ Talents/AstrologianTalents.xml        天赋实现
├─ Afflictions/AstrologianAfflictions.xml 天赋用的隐藏增益
├─ Items/AstrologianItems.xml            专属装备（天球仪 / 凶星 / 地星 / 命运之轮）
├─ Textures/                              图片素材（本地提供，不在仓库中）
│  ├─ JobIcon.png / JobIconSmall.png     职业图标（128×128 / 28×28）
│  ├─ Astrolabe / EarthlyStar / WheelOfFortune / Malefic.png   物品贴图（64×64）
│  ├─ Afflictions/                       增益图标（128×128）
│  └─ Talents/                           天赋图标（128×128）
└─ Text/
   ├─ SimplifiedChinese/SimplifiedChinese.xml
   └─ English/English.xml
```

## 免责声明

本模组为**非官方粉丝作品**，与 Square Enix、FakeFish、Undertow Games 均无关联。
《最终幻想14》及《潜渊症》的相关名称与设定归各自权利人所有。

## 致谢

参考了原版内容与以下模组的设计思路：

- [Talent Enhancement 天赋增强](https://steamcommunity.com/sharedfiles/filedetails/?id=2803561945)（天赋树结构风格）
- [Toxicologist Job](https://steamcommunity.com/sharedfiles/filedetails/?id=2993275947)
