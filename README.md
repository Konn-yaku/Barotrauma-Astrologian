...
- **与「天赋增强」（TE）风格一致** — 专精不互斥、解锁门槛与其保持一致
- **强度对标「天赋增强」（TE）** — 所有原创天赋的数值以 TE 为基准，而不是原版
- **全队常驻增益** — 奥秘卡不做抽卡、不做随机目标，整局稳定生效，避免「需要战斗时抽到后勤卡」的错配
## 完成度
...
| 专精线三 · 会奶人的DPS（专属武器 + 地星 + 命运之轮 + 占卜） | 🚧 第 1~4 档完成，待游戏验证 |
| 数值基准 | ✅ 已对标「天赋增强」（TE） |
| 初级天赋 | 🚧 暂为占位内容 |
...
本模组的强度基准是 **「天赋增强」（TE，工坊 ID `2803561945`）**，而不是原版。
TE 的典型数值：技能加点上限 +45、伤害抗性 0.25~0.75、增伤最高 +50%、治疗 3/秒。

| 效果 | 本模组（星力后） | TE 参照 | 原版参照 |
|---|---|---|---|
| 伤害 | +35% | `warlord` +10~50%（五层） | 安全官 `implacable` +25%（限时·仅自己） |
| 攻速（近/远） | +40% / +25% | 近战攻速最高 +0.3 | 舰长天赋 +30% / +20% |
| 移速 / 游泳 | +35% / +45% | 游泳 +0.3 | 基因顶档 +25% / +50% |
| 伤害抗性 | +35% | `stonewall` 0.2→0.4 | 基因顶档 +25% |
| 维修 / 船壳 | +45% / +45% | — | 工程师 `stationengineer` +50% / +50% |
| 氧气消耗 | −35% | — | 基因 −20% ~ −100% |

> 需要说明的是，TE 的增伤/抗性天赋大多带**层数或时间限制**，
> 而本模组的卡是**整局永久 + 全队**。
> 所以虽然单卡数值看起来相近，实际总收益仍然偏高 —— 这是为了对齐
> TE 的整体强度水平而有意为之。
...
| 固化泡沫手榴弹（原版机械师大招） | `structuredamage="-200"` | 1000（封整艘船） | 1 |
| **地星** | `structuredamage="-200"` | **250（封一个舱室）** | **1** |

修复量取原版同一个值：**保证一次性把范围内的船壳修满**，不做部分修补。
代价是范围只有原版大招的 1/4，而且用一次就没了。
...
| 范围 | 400 |
| 减伤 | **65% 全伤害抗性** |
| 治疗 | 持续 20 秒；速率 3.75/秒（理论总量约 75 点伤害 + 25 点出血） |
| 持续 | 20 秒 |
| 消耗 | 一次性，用完需重新制作 |
| 目标 | **不筛选** —— 范围内所有角色，包括海怪与敌方人类 |

### 强度对照

| 来源 | 减伤 | 持续 | 获取 |
|---|---|---|---|
| 本模组「天星交错」 | 25% | 整个任务 | 点天赋即得 |
| 本模组「世界树之干」 | 25% | 整个任务 | 点天赋即得 |
| TE `stonewall` 坚如磐石 | 20% → 40% | 带层数 | 消耗层数 |
| TE `Repairandreinforcement` | 25% → 65% | 长期 | 需养成 |
| **命运之轮** | **65%** | **20 秒** | **消耗品** |

对标 TE 的中高档（0.65）。原版 `damage` 抗性的上限只有 0.5，
这里是**刻意高于原版**的——因为玩家要求整体强度对标 TE，
代价是“只有 20 秒 + 一次性消耗品”。
...
**二、为什么`targets`里什么都没写**

`NearbyCharacters` **没有任何队伍过滤属性** —— 字段全集里只有
`target` / `targets` / `targetidentifiers` / `targettags` 等，没有 `team`。
原版只能按物种过滤（`Watcher.xml` 用 `targets="human"`）。

本模组按设计意图**不筛选**：范围内所有角色都吃到 buff，
包括海怪与敌方人类。如果以后想改回「只对人类」，
在那一行加回 `targets="human"` 即可。
...
回血速率是 3.75/秒，是「阳星合相」（0.75/秒）的 5 倍，持续 20 秒。
但理论总量（约 75 点）需要你活满 20 秒才能吃到 —— 潜渊症战斗节奏很短，
实战里能回上二三十点已经很好了。觉得总量太高就把 `amount` 减半。

---

## 占卜（第三栏 · 第 4 档）

占星术士把星象读透：知道该打哪里、该怎么打。这是第三栏的终档。

| 项 | 值 |
|---|---|
| 医疗技能 | **+45** |
| 自身伤害 | 随医疗技能阶梯提升，**+10% ~ +30%** |
| 队友伤害 | 1000 范围内 **+10%**（离开范围约 4 秒后失效） |

### 伤害阶梯怎么来的

写了 5 个 `OnAttack` 组：基础一个无条件 +10%，
另 4 个各带一道递增的医疗门槛，每个 +5%。
同一组内的多个条件是与关系，不同组的加成会累加：

| 医疗技能 | < 40 | ≥ 40 | ≥ 60 | ≥ 80 | ≥ 100 |
|---|---|---|---|---|---|
| 伤害 | +10% | +15% | +20% | +25% | +30% |

技能条件的写法照原版 `mechanic_talent_items.xml`（`mechanical="lt 40"`）。

> ⚠️ 如果阶梯没生效（永远只 +10%，或直接跳到 +30%），
> 告诉我，改成固定一档即可。
...
| 来源 | 伤害加成 | 范围 |
|---|---|---|
| 本模组「太阳神之衡」 | +25~35% | 全队·永久 |
| TE `warlord` 暴力军阀 | +10~50% | 仅自己·带层数 |
| 原版安全官 `implacable` | +25% | 仅自己·限时 |
| **占卜（自己）** | **+10~30%** | 仅自己·常驻 |
| **占卜（队友）** | **+10%** | 1000 内 |

### 关于「对标闪电巫师 +45 电器」

确认过了：**你说得对**。「天赋增强」（TE）里的闪电巫师确实给了 45 点电器：

```xml
<!-- TE  TalentsEngineer.xml:294 -->
<CharacterAbilityGiveStat stattype="ElectricalSkillBonus" value="45" />
```

（原版闪电巫师并不给技能点，其效果是「10 秒未受攻击则下次近战伤害减免 80%、并眩晕攻击者 4 秒」；
TE 在保留它的同时额外加了这 45 点。）
